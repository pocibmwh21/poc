...
export class MyinfoComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.userInfo = this.user.userInfo;
        this.noImageAbbr = this.userInfo.firstName[0].toUpperCase() + this.userInfo.lastName[0].toUpperCase();
    ...
    onFilterChangPrimaryeSearch(event) {
        ...
        $('.added').remove();
        this.searchNewPrimarySkill = event.replace(/\w\S*/g, (w) => (w.replace(/^\w/, (c) => c.toUpperCase())));

        if (this.allPrimary.indexOf(this.searchNewPrimarySkill) !== -1 || event == '') {
            this.showAddNewPrimary = false;

        } else {
            // this.showAddNewPrimary = true;

            $('.prim-drop .filter-textbox')
                .append('<span style="margin-left:5%;margin-top:4%;display:inline-block;color: #909192;" class="added skill-text">No such skill please add new skill</span><img style="margin-top:4%" class="added float-right add-new-skill" src="../assets/img/plus-icon.png">')
    ...
    onFilterChangSecondarySearch(event) {
        ...
        this.searchNewSecondarySkill = event.replace(/\w\S*/g, (w) => (w.replace(/^\w/, (c) => c.toUpperCase())));

        if (this.allSecondary.indexOf(this.searchNewSecondarySkill) !== -1 || event == '') {
            this.showAddNewSecondary = false;

        } else {
            // this.showAddNewSecondary = true;
            $('.sec-drop .filter-textbox')
                .append('<span style="margin-left:5%;margin-top:4%;display:inline-block;color: #909192;" class="addedsec skill-text">No such skill please add new skill</span><img style="margin-top:4%" class="addedsec float-right add-new-skill" src="../assets/img/plus-icon.png">')
            //  document.getElementsByClassName("filter-textbox")[0].appendChild(btn)
        }
        $('.add-new-skill').click(function() {
    ...
    onSubmit() {
        ...
        if (this.editProfileForm.invalid || (this.primarySet.length == 0 || this.secondarySet.length == 0)) {
            return;
        } else {
            ...
            this.accountService.update(this.user.id, this.data)
                .pipe(first())
                .subscribe(
                    data => {
